  ...
  <div className="bg-gradient-to-r from-gold-500/10 to-gold-300/10 border border-gold-500/30 rounded-2xl mb-6 overflow-hidden">
    ...
    {expanded && (
      <div id="warmup-content" className="px-5 pb-5 space-y-3">
        {exercises.map((ex, i) => (
          <div key={i} className="flex items-center justify-between bg-carbon-900/50 rounded-xl p-3">
            <span className="text-gray-300">{ex.name}</span>
            <span className="text-gold-400 font-mono">{ex.prescription}</span>
          </div>
        ))}
        <button className="w-full bg-gold-500/20 hover:bg-gold-500/30 text-gold-400 font-semibold py-3 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-gold-400">
          Start Warm-up ▶
        </button>
      </div>
    )}
