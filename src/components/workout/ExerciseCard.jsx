...
import { Check, Info, Play, Zap } from 'lucide-react';
import ExerciseInfoModal from './ExerciseInfoModal';
import VideoModal from './VideoModal';
import { getExercise } from '../../data/exercises';
import { recordTutorialWatched } from '../../services/gamificationService';

const ExerciseCard = ({ exercise, index, completedSets, onStartSet, techMode, isActive, onXPEarned }) => {
  ...
  const handleShowVideo = (vid) => {
    // Use provided video, exercise database video, or search YouTube
    const videoToShow = vid || exerciseData?.videoId;
    if (videoToShow) {
      setVideoId(videoToShow);
      setShowVideoModal(true);
      setShowInfoModal(false);
    } else {
      // No video available - open YouTube search as fallback
      const searchQuery = encodeURIComponent(`${exercise.name} exercise tutorial`);
      window.open(`https://www.youtube.com/results?search_query=${searchQuery}`, '_blank');
    }
  };

  const handleVideoWatched = () => {
    const result = recordTutorialWatched();
    onXPEarned?.(result.earnedXP, 'Watched tutorial');
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* XP indicator */}
        {exerciseData && (
          <div className="flex items-center gap-1 text-gold-400 text-xs mb-3">
            <Zap size={12} />
            <span>+{exerciseData.xpValue} XP per set</span>
          </div>
        )}
            ...
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-semibold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-gold-400 ${
                completedSets.includes(i)
                  ...
                  : 'bg-carbon-700 text-white hover:bg-carbon-600 active:scale-95'
              }`}
