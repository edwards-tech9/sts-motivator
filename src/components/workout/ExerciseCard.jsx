...
const ExerciseCard = ({ exercise, index, completedSets, onStartSet, techMode, isActive }) => {
  ...
  return (
    <div
      className={`bg-slate-800/50 rounded-2xl p-5 mb-4 border-2 transition-all ${
        isActive
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <span
            className="w-8 h-8 bg-slate-700 rounded-lg flex items-center justify-center text-gray-400 font-bold text-sm"
            aria-hidden="true"
          >
            {index}
          </span>
        ...
        <div className="flex gap-2">
          <button
            className="p-2 bg-slate-700/50 rounded-lg text-gray-400 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-orange-400"
      ...
      <div className="flex gap-2" role="group" aria-label="Sets">
        {Array.from({ length: exercise.sets }, (_, i) => (
          <button
            ...
            className={`flex-1 py-3 rounded-xl font-semibold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-orange-400 ${
